...
//   Implements Round-to-Nearest-Even (RNE).
// =========================================================================
...
module q16_to_fp32 (
    ...
    function automatic logic [4:0] clz32(input logic [31:0] val);
        logic [4:0] res;
        if (val[31:16] == 16'b0) begin res[4] = 1'b1; val = val << 16; end else res[4] = 1'b0;
        if (val[31:24] ==  8'b0) begin res[3] = 1'b1; val = val <<  8; end else res[3] = 1'b0;
        if (val[31:28] ==  4'b0) begin res[2] = 1'b1; val = val <<  4; end else res[2] = 1'b0;
        if (val[31:30] ==  2'b0) begin res[1] = 1'b1; val = val <<  2; end else res[1] = 1'b0;
        if (val[31]    ==  1'b0) begin res[0] = 1'b1;                  end else res[0] = 1'b0;
        return res;
    endfunction
