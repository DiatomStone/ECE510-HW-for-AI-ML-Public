...
The new throughput FLOPS would change after the putting GELU on hardware. A deeper inspection is required since if this is scaled 
up to a large configuration GELU might not be the biggest bottleneck. Currently ~10 seconds is required to transfer ~5.3 GB *AND* perform operations. 
So a minium of ~500 MB/s is needed. Axi stream or PCIe is a good candidate, with AXI-lite likely being too slow. 
Tentatively PCIe is selected as the hardware interface to learn more about PCIe. 
