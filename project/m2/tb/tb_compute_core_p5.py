...
# Drive all inputs back to back and collect overlapping outputs
    for x_float in inputs:
        dut.valid_in.value = 1
        dut.x.value = to_fixed(x_float) & 0xFFFFFFFF
        await RisingEdge(dut.clk)
        
        # Check for outputs emerging while we are still driving new inputs
    ...
    # Collect the remaining outputs to flush the pipeline
