...
# x8 / x32 = the parameterized gelu_top built at NUM_LANES=8 or 32 (8- or
...
# Select the design config by variant (2nd arg, default = synth/config.json):
#   default -> synth/config.json  (x8 streaming gelu_top — the M4 deliverable config)
#   v1  -> gelu_top         (single gelu_fp32 kernel, 32-bit AXI-Stream, no macros)
#   x8  -> gelu_top @ 8  (8 parallel gelu_fp32 pipelines, 256-bit AXI-Stream)
#   x16 -> gelu_top @ 16 (16 parallel gelu_fp32 pipelines, 512-bit; relaxed area)
#   x32 -> gelu_top @ 32 (32 parallel gelu_fp32 pipelines, 1024-bit AXI-Stream)
#   dma -> gelu_dma_top     (DMA-wrapped design; inferred 256-deep buffers)
# An explicit CONFIG=... env var overrides the variant selection.
case "$VARIANT" in
    default) DEFAULT_CONFIG="$SCRIPT_DIR/../synth/config.json" ;;   # x8 streaming (= config_x8.json)
    v1)  DEFAULT_CONFIG="$SCRIPT_DIR/../synth/config_v1.json" ;;
    x32) DEFAULT_CONFIG="$SCRIPT_DIR/../synth/config_x32.json" ;;
    x16) DEFAULT_CONFIG="$SCRIPT_DIR/../synth/config_x16.json" ;;
    x8)  DEFAULT_CONFIG="$SCRIPT_DIR/../synth/config_x8.json" ;;
    dma) DEFAULT_CONFIG="$SCRIPT_DIR/../synth/config_dma.json" ;;
    *)
        echo "Unknown VARIANT '$VARIANT' (expected: default | v1 | x8 | x16 | x32 | dma)"
        exit 1
        ;;
esac
...
case "$MODE" in
    ...
    smoke)
        ...
        ;;
    *)
        echo "Usage: $0 [synth|full|smoke] [default|v1|x8|x32|dma]"
        echo "  Mode (1st arg):"
        ...
        echo "  Variant (2nd arg, default = synth/config.json, x8 streaming):"
        echo "    default : synth/config.json (x8 streaming gelu_top — M4 deliverable)"
        echo "    v1  : gelu_top      (single kernel, 32-bit AXI-Stream, no macros)"
        echo "    x8  : gelu_top @ NUM_LANES=8  (8 parallel pipelines, 256-bit)"
        echo "    x16 : gelu_top @ NUM_LANES=16 (16 parallel pipelines, 512-bit)"
        echo "    x32 : gelu_top @ NUM_LANES=32 (32 parallel pipelines, 1024-bit)"
