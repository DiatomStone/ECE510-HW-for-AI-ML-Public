...
# `@profile` is injected as a builtin by kernprof / memory_profiler (and by the
# cocotb tb). Provide a no-op fallback so the module also runs under plain
# `python` and `python -m cProfile train.py` (cProfile needs no decorator).
...
@profile
def cast_fp32_roundtrip(x):
    """Emulate the accelerator's host-side precision cast around the GELU offload:
    fp64 -> fp32 (DMA into the kernel) then fp32 -> fp64 (DMA back out). Isolated
    as its own profiled function so a cProfile / line-profiler run of the base
    software measures this cast cost directly, instead of the noisy wall-clock
    perf_counter timing used in the cocotb testbenches (acc["conv_us"])."""
...
def ff_forward(x, W1, b1, W2, b2):
    h = x @ W1 + b1
    h = cast_fp32_roundtrip(h)        # host-side fp64<->fp32 cast (accelerator I/O)
    h_act = gelu(h)
    out = h_act @ W2 + b2
