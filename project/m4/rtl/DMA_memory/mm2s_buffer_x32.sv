// =========================================================================
// Module:  mm2s_buffer_x32
// Project: GELU Activation Kernel - ECE 410/510, M3
//
// Description:
//   Wide, macro-free input DMA buffer for the 32-lane kernel. Accepts AXI4-MM
//   write bursts from the DMA engine and streams stored beats out to the
//   kernel over a DATA_W-wide AXI4-Stream (default 1024 bits = 32 FP32 lanes).
//
//   Unlike mm2s_buffer (which was backed by the sky130 SRAM macro via
//   openram_1k_wrap), this version stores data in an inferred register-array
//   FIFO — pure standard cells, no SRAM macro, no blackbox, no OpenLane MACROS
//   config. It is sized SHALLOW (DEPTH beats) rather than deep: a streaming
//   feed-through kernel only needs enough elastic buffering to cover DMA burst
//   jitter and the 12-cycle pipeline latency, not 1024-deep residency.
//
