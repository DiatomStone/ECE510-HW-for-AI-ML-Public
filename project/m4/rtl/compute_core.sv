// =========================================================================
// Module:  compute_core  (High-Accuracy 24-Segment Parallel PWL GELU)
// Project: GELU Activation Kernel - ECE 410/510, M3
//
// Description:
//   4-stage pipelined piecewise-linear (PWL) approximation of the GELU
//   activation function. Operates on Q16.16 signed fixed-point values.
//   Uses 20 non-uniform segments with higher density near the high-curvature
//   center region for maximum accuracy. Full throughput: one result per cycle.
//
//   Stage 1: Boundary decode — compares x against segment boundaries.
//   Stage 2: Coefficient select — maps segment hit to slope (m), intercept
//             (b), and segment base (t).
//   Stage 3: Multiply — computes m * (x - t).
//   Stage 4: Add, saturate, output — adds b, clamps to signed Q16.16.
//
// Parameters:
//   DATA_WIDTH - bit width of input/output (default 32, matches Q16.16)
//   FRAC_BITS  - fractional bit count for Q format (default 16)
//   PIPE_DEPTH - number of pipeline stages (default 4; must match stage count)
//
// Clock Domain:
//   Single clock domain (clk). All sequential logic on posedge clk.
//
// Reset:
//   Synchronous, active-high (rst). Clears valid pipeline and output register.
//
// Ports:
//   clk       - input,  1          : System clock
//   rst       - input,  1          : Synchronous active-high reset
//   valid_in  - input,  1          : Asserted when x holds a valid Q16.16 operand
//   x         - input,  DATA_WIDTH : Signed Q16.16 GELU input value
//   valid_out - output, 1          : Asserted when out holds a valid Q16.16 result
//   out       - output, DATA_WIDTH : Signed Q16.16 GELU(x) result
// =========================================================================
