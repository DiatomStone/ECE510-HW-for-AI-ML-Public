...
Buffer depth is 256 words (shrunk from 1024 so gelu_dma_top routes in OpenLane),
so each GELU tensor is streamed in CHUNK-sized DMA bursts (CHUNK ≤ 256). The
pure-software transformer.forward() output is the reference; final logits and
per-layer GELU accuracy are checked.
...
# single burst is at most 256 beats (beats = AxLEN + 1). Also <= buffer depth (256).
CHUNK              = 256
GELU_THRESHOLD     = 0.05

assert CHUNK <= 256, "CHUNK exceeds the AXI4 max burst length (AxLEN is 8-bit)"
assert CHUNK <= 256, "CHUNK must fit in the 256-deep mm2s/s2mm buffers"
