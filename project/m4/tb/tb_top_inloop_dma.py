...
every beat packs 32 IEEE-754 FP32 operands (lane i at bits [i*32 +: 32]).
      ...
      host (fp64->fp32)
        -> AXI4-MM write burst (1024-bit) -> mm2s_buffer (wide FIFO)
        -> AXI4-Stream (1024-bit)          -> gelu_top (32 lanes)
        -> AXI4-Stream (1024-bit)          -> s2mm_buffer (wide FIFO)
        -> AXI4-MM read burst (1024-bit)  -> host (fp32->fp64)

DUT: gelu_dma_top (rtl/DMA_memory/gelu_dma_top.sv) with DEPTH=256 buffers.
...
async def dma_write_burst(dut, beat_words):
    """Burst-write a list of DATA_W-bit beat words into mm2s_buffer."""
    n = len(beat_words)
...
async def dma_read_burst(dut, n_beats):
    """Burst-read n_beats DATA_W-bit beat words from s2mm_buffer."""
    data_out = []
    ...
    # s2mm_buffer has combinational first-word-fall-through rvalid (high every
