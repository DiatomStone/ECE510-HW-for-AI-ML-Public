"""
tb/tb_top_inloop_x32.py — in-loop, timed co-sim of gelu_top_x32 (32 parallel lanes).

This is the 32-lane / parallel-pipeline revision of tb_top_inloop.py. It runs
the same M1 transformer forward pass and offloads EVERY GELU to the hardware,
but the DUT is gelu_top_x32, which packs NUM_LANES (=32) IEEE-754 FP32 operands
into every AXI-Stream beat and feeds 32 gelu_fp32 pipelines in lockstep
(see rtl/interface_x32.sv).
...
# ---------------------------------------------------------------------------
# Parallelization — must match gelu_top_x32 NUM_LANES parameter.
...
# ---------------------------------------------------------------------------
# hw_gelu — offload one GELU activation tensor to gelu_top_x32 over AXI-Stream.
#
...
# ---------------------------------------------------------------------------
@cocotb.test()
async def test_gelu_top_x32_inloop(dut):
    ...
    assert bus_lanes == NUM_LANES, (
        ...
        f"but NUM_LANES={NUM_LANES}. Rebuild with gelu_top_x32 / matching NUM_LANES.")
    ...
    # read-only identity register: lane count (interface_x32 reg 0x04)
