"""
tb_axis_interface_x32.py  —  AXI protocol testbench for
gelu_axi_stream_interface_x32 (32 parallel gelu_fp32 pipelines).

This is the wide / parallel revision of tb_axis_interface.py. The DUT now has a
NUM_LANES*32 = 1024-bit AXI-Stream: every beat packs NUM_LANES (=32) IEEE-754
FP32 operands, one per lane, processed by 32 gelu_fp32 pipelines in lockstep
(see rtl/interface_x32.sv). The AXI-Lite control path is unchanged, with one
addition: read-only register 0x04 reports NUM_LANES.

Lane packing (matches interface_x32): lane i occupies tdata[i*32 +: 32], so
lane 0 is the least-significant 32 bits of the 1024-bit word.

Like the v1 testbench, this drives the raw AXI ports directly (no cocotbext);
only the data plane widens to a packed multi-lane word per beat.

AXI-Lite handshake timing and the Icarus/VPI NBA note are identical to v1.
"""
...
async def test_accuracy_sweep(dut):
    ...
    print(f"  Lanes     : {NUM_LANES} (1024-bit AXI-Stream)")
